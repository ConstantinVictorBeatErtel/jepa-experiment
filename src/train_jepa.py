"""Train a tiny JEPA-style vision model on CIFAR-10 or STL-10."""
...
from src.losses import get_latent_loss
from src.models import build_jepa_model
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Override config values like model.mask_ratio=0.8. Can be passed multiple times.",
    )
    parser.add_argument(
        "--mask-ratios",
        type=float,
...
def train_one_run(config: Dict, mask_ratio: float) -> Dict:
    """Train a single JEPA run for one mask ratio."""
    run_dir = create_run_dir(config, objective="jepa", mask_ratio=mask_ratio)
    ...
    model = build_jepa_model(config).to(device)
    latent_loss = get_latent_loss(config["training"]["latent_loss"])
    ...
    for epoch in range(1, int(config["training"]["epochs"]) + 1):
        ...
        running_loss = 0.0
        total_examples = 0
        progress_bar = tqdm(train_loader, desc=f"JEPA Epoch {epoch}", leave=False)
        for images, _, _ in progress_bar:
            ...
            with get_autocast_context(device, amp_enabled):
                ...
                loss = latent_loss(outputs["predictions"], outputs["targets"])
